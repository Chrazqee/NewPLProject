...
class NCaltech101(NCaltech101Base):
    def __init__(self, bins: int = 3, data_path: str = '', data_type='training', split_ratio=None,
        ...
        for i, cls in enumerate(self.cls_list):
            ...
            test_split_list_dvs = dvs_file_list[cut_train + cut_val:]
            ...
            test_split_list_img = [(file.split('.'))[0] + ".jpg" for file in dvs_file_list[cut_train + cut_val:]]
            ...
            test_split_list_annotation = ["annotation_" + file[6: 10] + "_box.npy" for file in
                                          dvs_file_list[cut_train + cut_val:]]
            ...
            if data_type == 'training':
                ...
                for file_dvs, file_img, file_annotation in zip(test_split_list_dvs, test_split_list_img,
                                                               test_split_list_annotation):
                    self.dvs_filelist.append(os.path.join(self.file_path_dvs, cls, file_dvs))
                    self.img_filelist.append(os.path.join(self.file_path_img, cls, file_img))
                    self.targets.append(os.path.join(self.file_path_annotation, cls, file_annotation))
            else:
    ...
    def __getitem__(self, index):
        ...
        """
        file_pth_dvs = self.dvs_filelist[index]
        file_path_img = self.img_filelist[index]
        bbox = self.targets[index]
    ...
    def generate_hist(self, x, y, t, p):
        hist = StackedHistogram(self.bins, self.resize_scale, self.resize_scale, self.resize_scale)
        img_ = hist.construct(x, y, p, t)
        return img_
...
if __name__ == "__main__":
    ...
    # ncaltech101 = NCaltech101(data_path=_data_path, data_type='validation')
    ...
    lif_node = LIFNode(tau=2.0, detach_reset=True, backend='torch', step_mode='m', store_v_seq=False)
    data_nums = ncaltech101.data_num
    for idx in range(data_nums):
        file_path = ncaltech101.dvs_filelist[idx]
        file_name = file_path.split('/')[-1]
